...
const Sidebar = () => {
  ...
  const topSectionItems = SidebarMenuItems.filter((item) => item.section === "top");
  const bottomSectionItems = SidebarMenuItems.filter((item) => item.section === "bottom");

  const handleItemClick = (item: typeof SidebarMenuItems[0], e: React.MouseEvent) => {
    if (item.isLogout) {
      e.preventDefault();
      logout();
    }
